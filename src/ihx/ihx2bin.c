// ihx2bin by Robert van Engelen
// BSD-3 license, May 10, 2021, last updated Oct 6, 2025
// build: cc -o ihx2bin ihx2bin.c
// usage: ihx2bin [-g] [file.ihx|-] [file.bin]
// option -g: gaps allowed in ihx, otherwise helps to catch assembly/link errors
...
static int ihxgaps = 0;
static char ihxbuffer[BUF_SIZE];
static unsigned int ihxforward = 0;
static unsigned int ihxaddress = 0;
static unsigned int ihxlinelen = 0;
static unsigned int ihxlinepos = 0;
static unsigned int ihxlinenum = 0;
...
int getbyte(FILE *ihx) {
  ...
  while (ihxlinepos >= ihxlinelen) {
    ...
    if (ihxaddress == 0) {
      ...
      if (ihxgaps && ihxaddress < address) {
        ++ihxaddress;
        ihxforward = address - ihxaddress;
        return 0;
      }
