...
User-defined machine `CODE` words in ForthMSX should not use `jp` absolute
jumps or `call` calls to other Forth words.  However, for the ForthMSX assembly
code internals the situation is different, because the assembler's forth.rel
file has all of the necessary relocation information about absolute jumps and
calls to relocate; in addition, forslice also supports a final last
unconditional `jr` relative jump to another Forth word that will be analyzed
and adjusted during relocation.
